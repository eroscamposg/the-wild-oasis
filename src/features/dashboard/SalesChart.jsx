...
function SalesChart({ bookings, numDays }) {
  ...
  // Create a map of bookings grouped by date (O(m) - single pass through bookings)
  const bookingsByDate = bookings.reduce((acc, booking) => {
    const dateKey = format(new Date(booking.created_at), 'MMM dd');
    if (!acc[dateKey]) {
      acc[dateKey] = { totalSales: 0, extrasSales: 0 };
    }
    acc[dateKey].totalSales += booking.total_price;
    acc[dateKey].extrasSales += booking.extras_price;
    return acc;
  }, {});

  // Map dates using pre-computed values (O(n) - single pass through dates)
  const data = allDates.map(date => {
    const label = format(date, 'MMM dd');
    return {
      label,
      totalSales: bookingsByDate[label]?.totalSales || 0,
      extrasSales: bookingsByDate[label]?.extrasSales || 0,
    };
  });
